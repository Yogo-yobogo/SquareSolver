...
void solve_square(struct Data_for_square* data);
...
void begin_solve_square() // программа решающая уравнение вводимое с клавиатуры
{
    ...
    printf("You entered the equation % lg*x^2%+lg*x%+lg = 0\n", data.a, data.b, data.c);
    solve_square(&data);
    print_roots(&data);
}


void solve_square(struct Data_for_square* data) // решает уравнение возвращяет количество корней, решение записывает в структуру data в х1 и х2
{
    ...
    if (is_zero(data->a))
    {
        data->num_of_roots = solve_lin_eq(data->b, data->c, &(data->x1));
        return;
    }
...
void print_roots(struct Data_for_square* data)
{
    ...
    case INF_NUM_OF_ROOTS:
        printf("Root of this equation is any number");
        break;
